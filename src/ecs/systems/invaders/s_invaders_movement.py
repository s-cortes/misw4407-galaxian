...
from src.ecs.components.base import CLevel, CTransform, CVelocity, CSurface, CAnimation
...
def system_invaders_movement(
    world: World, delta_time: float, level: CLevel
):
    if any([level.paused, level.completed]):
        return

    components = world.get_components(
        CVelocity, CTransform, CSurface, CAnimation, CInvaderState, CTagInvader
    )
    ...
    for _, (
        ...
        if c_invader_state.state != InvaderState.MOVE:
            ...
            c_surface.surf = pygame.transform.rotate(
                c_invader.single_surface.copy(), c_invader.angle
            )
